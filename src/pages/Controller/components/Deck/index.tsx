...
import { EFFECT_SHORTCUTS } from "@/pages/Controller/hooks/useEffectShortcuts";
...
const Deck = ({ localStorageKey, deckId, className, initialPaused = false }: DeckProps) => {
  ...
  const [activeEffect, setActiveEffect] = useState<string>("1");

  const applyEffect = useCallback(
    (effectKey: string) => {
      const shortcut = EFFECT_SHORTCUTS.find((s) => s.key === effectKey);
      if (!shortcut) return;
      // すでにアクティブなエフェクトを再度押したらリセット
      const isActive = activeEffect === effectKey && effectKey !== "1";
      if (isActive || Object.keys(shortcut.filters).length === 0) {
        deckAPIRef.current?.clearFilters();
        setActiveEffect("1");
      } else {
        deckAPIRef.current?.setFilters(shortcut.filters);
        setActiveEffect(effectKey);
      }
    },
    [deckAPIRef, activeEffect]
  );
  ...
  return (
    <div className={`${styles.deck} ${className}`}>
      ...
      <fieldset className={styles.controlFieldset}>
        <legend>Effects <span className={styles.effectHint}>(Shift+数字)</span></legend>
        <div className={styles.effectButtons}>
          {EFFECT_SHORTCUTS.map((shortcut) => (
            <button
              key={shortcut.key}
              type="button"
              className={`${styles.effectButton} ${activeEffect === shortcut.key ? styles.effectButtonActive : ""}`}
              onClick={() => applyEffect(shortcut.key)}
              title={`Shift+${shortcut.key}`}
            >
              {shortcut.label}
            </button>
          ))}
