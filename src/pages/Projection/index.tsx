...
const ProjectionPage = () => {
  ...
  const [ch2Filters, setCh2Filters] = useState<Record<string, string>>({});
  ...
  const ch2Events = useMemo(
    () => ({
      onFiltersChange: (filters: Record<string, string>) => {
        setCh2Filters(filters);
      },
    }),
    []
  );
    ...
    <>
      ...
      <div
        style={{
          position: "fixed",
          inset: 0,
          // デッキ3: opacityフィルターをそのままz=2の不透明度として使用
          opacity: Number(ch2Filters.opacity ?? 0),
          zIndex: 2,
          filter: Object.entries(ch2Filters)
